...
const RegistrationAdd = async (req, res) => {
    try {
        const { Fname, Lname, Email, MNumber, Address, AdhaarNo, Gender, course, Year, employed, placement, presentOrgani, CurrentDesignation } = req.body;
        ...
        if (!Fname) {
            ...
            return res.status(401).send("Gender is required");
        } else if (!Email) {
        ...
        if (!RegistrationCheck.length) {
            ...
            return res.status(500).send("Your are allready Submited")
        }

    } catch (error) {
        console.log(error);
        return res.status(500).send("There was some error, please check your details")
    }
}
